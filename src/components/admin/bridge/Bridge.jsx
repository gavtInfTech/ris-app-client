...
export default function Bridge(props) {
  ...
  const handleDeleteClick = (id) => async () => {
    ...
    if (row.date.toDateString() !== today.toDateString()) {
      if (auth.role === "Администратор") {
        ...
        try {
          isEditAllowed
            ? await api.delete(`/bridges/delete/${id}`)
            : await api.post(`/bridges/deleteWithConfirm/${id}`);
  ...
  const processRowUpdate = async (newRow) => {
    ...
    if (updateFlag) {
      try {
        if (isEditAllowed) {
          let res = await api.post("/bridges/change", {
            ...updatedRow,
      ...
      try {
        let res = await api.post("/bridges/add", {
          ...updatedRow,
