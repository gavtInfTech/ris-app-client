...
const WeatherBanner = ({ forecastNow, unit, locale }) => (
  <div>
    <h5>
      {`${moment.unix(forecastNow.dt).locale(locale).format('dddd h:mm a')}, ${forecastNow.desc}`}
    </h5>
    <BannerContainer>
      ...
      <DetailContainer>
        <InfoText>
          Облачность: <b>{forecastNow.clouds}%</b>
        </InfoText>
        <InfoText>
          Давление: <b>{forecastNow.humidity}%</b>
        </InfoText>
        <InfoText>
          Скорость ветра:{' '}
          <b>
            {forecastNow.wind} {unit === 'metric' ? 'м/с' : 'mph'}
          </b>
        </InfoText>
      </DetailContainer>
    </BannerContainer>
